...
if [ ! -d "$BACKUP_DIR" ]; then
    log "Output folder $BACKUP_DIR does not exist."
    exit 1
fi

if [ ! -f "/usr/local/bin/pishrink" ]; then
    log "PiShrink not found at /usr/local/bin/pishrink."
    log "Please install PiShrink by following the instructions at: https://github.com/Drewsif/PiShrink"
    exit 1
else
    if ! command -v pigz &> /dev/null; then
        log "pigz not found. Install pigz by running: apt install pigz"
        exit 1
    fi
fi
